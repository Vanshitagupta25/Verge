...
import type { Channel } from '@/src/app/page';
...
export default function ChannelOnboarding({
  ...
  const handleJoinAll = async () => {
    ...
    const allUnjoinedIds = unjoinedChannels.map((channel) => channel._id);
  ...
  return (
    <div className="min-h-screen w-full bg-[#111827] text-white flex items-center justify-center p-3 sm:p-6 md:p-8 overflow-x-hidden">
      <motion.div
        ...
        className="w-full max-w-4xl bg-[#111827] border border-[#374151] rounded-xl p-4 sm:p-6 md:p-10 space-y-6 md:space-y-8 shadow-2xl"
      >
        {/* Header Section */}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4 max-h-[50vh] sm:max-h-none overflow-y-auto sm:overflow-visible pr-1 sm:pr-0">
